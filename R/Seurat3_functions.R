...
#' @param group.by character vector of column group labels
#' @param save.path folder to save
DoHeatmap.matrix <- function (object, features = NULL, cells = NULL, 
                              no.legend =F, group.by = NULL, group.bar = TRUE, 
                              ...
                              width=10, height=7,res=600,...) 
{
    ...
    disp.max <- disp.max %||% ifelse(test = slot == "scale.data", 
                                     yes = 2.5, no = 6)
    possible.features <- rownames(x = object)
    ...
    data <- as.data.frame(x = as.matrix(x = t(x = object[features, cells, drop = FALSE])))
...
eulerr <- function(df, shape =  "circle", key = NULL,cut_off = "avg_logFC",
                   cut_off_value = 0.05, do.lenged = TRUE,do.return = TRUE,
        ...
        if(do.print) {
            if(is.null(save.path)) save.path <- paste0("output/",gsub("-","",Sys.Date()),"/")
            if(!dir.exists(save.path)) dir.create(save.path, recursive = T)
            jpeg(paste0(save.path,"Venn_",file.name,"_",cut_off,
...
FeaturePlot.1 <- function (object, features, dims = c(1, 2), cells = NULL, 
                           ...
                           label.size = 4,strip.text.size = 15, repel = FALSE, ncol = NULL, combine = TRUE, border = FALSE,
    ...
    if (!is.null(x = split.by) & length(features) == 1) {
        plot <- plot + facet_grid(facets = . ~ split.by)
    }
    if (is.null(x = split.by) & length(features) > 1) {
        plot <- plot + facet_wrap(facets = features ~.,ncol = ncol)
    }
        plot <- plot + theme(strip.background = element_blank(),
                  strip.text =  element_text(face="plain",size=strip.text.size))
    if(border) plot = plot + theme(panel.border = element_rect(colour = "black"))
...
FgseaBarplot <- function(stats=res, pathways=hallmark, nperm=1000,cluster = 1,
    ...
    fgseaRes <- fgsea(pathways=pathways, stats=geneRank, nperm=nperm)
    print(dim(fgseaRes))
...
FgseaDotPlot <- function(stats=results, pathways=NULL, 
                         nperm=1000,size = "-log10(pval)", Rowv = NULL,Colv = NULL,
                         font.ytickslab = 15,
                         fill = "NES",title="",order.row.by = c(1,"pval"),
    ...
    for(i in seq_along(clusters)){
        ...
        fgseaRes[[i]] <- fgseaMultilevel(pathways=pathways, stats=geneRank, nperm=nperm)
        fgseaRes[[i]] = as.data.frame(fgseaRes[[i]])
        fgseaRes[[i]] = fgseaRes[[i]][,c("pathway","pval","padj","NES")]
        if(clusters[i] == order.row.by[1]) {
            order.row = fgseaRes[[i]][order(fgseaRes[[i]][,order.row.by[2]],
    ...
    if(isTRUE(Rowv)) {
        ...
        order.row = rownames(mtx_fgseaRes)[rowInd]
    } else {
        order.row = order.row[order.row %in% df_fgseaRes[,"pathway"]]
    }
    if(isTRUE(Colv)) {
        ...
        order.col = colnames(mtx_fgseaRes)[colInd]
    }
    df_fgseaRes[,"pathway"] %<>% as.factor
    df_fgseaRes[,"pathway"] %<>% factor(levels = order.row)
...
FindMarkers.UMI <- function (object, ident.1 = NULL, ident.2 = NULL, group.by = NULL, 
    ...
    de.results <- FindMarkers(
        ...
        pseudocount.use = pseudocount.use,
        ...)
    colnames(de.results)[colnames(de.results) =="avg_diff"] = "avg_logFC"
    ...
    if(slot == "data"){
        ...
        if(assay == "SCT"){
            avg_UMI.1 <- Matrix::rowMeans(exp(x = data.use[, ident.1]))
            avg_UMI.2 <- Matrix::rowMeans(exp(x = data.use[, ident.2]))
        }
    }
    else if(slot == "counts"){
...
FindPairMarkers <- function(object, ident.1, ident.2 = NULL, features = NULL,return.thresh = 0.05,
                            ...
                            min.cells.feature = 3,min.cells.group=3, pseudocount.use = 1, slot = "data",
...
MakeUniqueGenes <- function(object, features){
    ...
    if(class(object) == "Seurat") {
        scale.data = object@assays$RNA@scale.data
    ...
    if(class(object) == "Seurat") {
        object@assays$RNA@scale.data = scale.data
        return(object)
    } else if(class(object) %in% c("data.frame","matrix")){
        return(scale.data)
    }
...
TSNEPlot.1 <- function(object,dims = c(1, 2),cells = NULL,cols = NULL,pt.size = NULL,
                       ...
                       repel = TRUE,alpha = 0.85, strip.text.size = 15,title.size = 15,
                       legend.size = 15,
                       cells.highlight = NULL,cols.highlight = 'red',sizes.highlight = 1,
                       na.value = 'grey50',combine = TRUE,ncol = NULL,title = NULL,legend.title = NULL,
...
Singler.colors <- c("#7FC97F","#BEAED4","#FDC086","#386CB0","#F0027F",
                    ...
                    "#E31A1C","#FF7F00","#6A3D9A","#8DA0CB","#377EB8",
...
UniqueName <- function(object, fileName = NULL, unique.name = T){
    ...
    if(unique.name %in% colnames(object@meta.data)) {
